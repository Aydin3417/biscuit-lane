...
Everything ships as **one self-contained HTML file**. No audio files, no
libraries, and one picture — every animal, biscuit, thorn and sound
effect is drawn or synthesised at runtime, and the only artwork in the
bundle is the logo, inlined.
...
src/
  ...
  js/
    ...
    02-logo.js     GENERATED by tools/icon.js: the brand mark as a data
                   URI, the same crop every store icon is cut from
...
test/              see below
tools/             the harness, the capture tools, and one patch
                   script per change, kept as a record
android/           the native shell, generated by Capacitor
ios/               the same, for Xcode. Portrait-locked, arm64, icons
                   and launch images drawn by tools/icon.js
store/             listing copy in both languages, the privacy policy,
                   and the checklist of what only an account holder can
                   do — see store/checklist.md
```
...
```bash
npm test                   # everything below that gates a commit
npm run test:node          # integrity, engine soak, all four difficulty curves
npm run test:check         # duplicate names, unresolved calls and
...
node tools/store.js        # store screenshots at Play and App Store
                           # sizes, and the 1024x500 feature graphic
...
`npm run test:browser` is the browser layer: it puts a server up, loads
the built game in an iframe and runs 39 checks, driving the real thing
through `window.BL`. It needs nothing running in another terminal.
`test/integration.html` is the same suite with a human looking at it.
They cover:
...
The browser layer had 39 checks and a runner, and neither had been run
for some time. Not because anything failed — because nothing could
start them.

Every tool in `tools/` that drives a browser opened with the same four
lines:
...
Playwright borrowed from a sibling project's `node_modules`, on one
particular machine, belonging to one particular developer. A Chrome at a
path that exists on Windows and nowhere else. And underneath, a base URL
of `http://localhost:5173` — a port this repository has never served, and
does not know about: `tools/serve.js` listens on 8181.

So on a fresh clone all nine of them threw at `require`. If you happened
to have the sibling project, they got further and then failed to
connect. The comment above those lines said the arrangement was
deliberate — the game has no dependencies, so the dev tools borrow one —
and the intent was right. The execution made the tools unrunnable by
anybody who was not the person who wrote them, which for a two-person
project is half the team.

What it cost is the part worth stating. `tools/browser.js` exists because
"a test nobody is made to run is a test that quietly stops being true" —
its own docstring says so. It was then left in a state where nobody
could run it. The 39 checks are the only ones in the project that touch
a real document: focus order, sheets that can be reached, a level played
through the interface, whether a frame sends NaN to a canvas. All of it
had been dark.

There is a `package.json` now. Playwright is a devDependency, the
fallbacks still work through `PLAYWRIGHT` and `CHROME` for anyone who
wants to point them elsewhere, and the four repeated lines are one
`tools/_pw.js` — the same reason `test/_modules.js` exists, which says
in its own docstring that a list repeated seven times is a list that
will be wrong. This was a list repeated nine times.

`tools/serve.js` also exports its server now, so `npm run test:browser`
and `npm run hunt` put one up themselves rather than failing because
nobody had a second terminal open. If a server is already listening on
the port, that one is used and starting is a no-op.

All 39 pass. `hunt` finds nothing. Both run in CI on every push, along
with a check that the committed `index.html` still matches what `src/`
builds — because the artifact is committed, and an artifact that has
drifted from its source is worse than no artifact.

## What a frame costs in a real browser

The frame budget was measured by the harness and asserted in the suite,
but `tools/frame.js` — which times the same thing in an actual browser
rather than a simulated one — was among the tools that could not start.
It runs now:

| | median | p90 | worst |
|---|---|---|---|
| quiet frame | 0.95ms | — | — |
| a realistic clear, 330 particles | 1.4ms | 1.8ms | 4.3ms |
| a whole-board combo, 803 particles | 2.2ms | 2.7ms | 8.8ms |

Against 16.7ms at 60fps, on a desktop. That last number is the
qualifier and it is worth keeping in front of us: this is a desktop
browser, not a phone. Nothing here has been profiled on the hardware it
is meant to run on. The margin looks large enough that it probably
survives the trip, but "probably" is not what the rest of this project
measures things to.

## Something you install

The game is a web page, which is what made it good — one file, no build
chain, no engine, and a service worker that keeps it working on the
underground. It is also what stopped it being a product: iOS has no
purchase API for web apps and no App Store presence for them, so a
...
```bash
npm run sync           # build, pack, and copy into the native projects
npm run android:apk    # a debug APK
npm run android        # open the project in Android Studio
npm run ios            # the same, in Xcode — needs a Mac
```

Two things the shell changes on purpose. The service worker is left out
of the native payload: inside a WebView the app is already local, so the
cache it manages is a second copy of files that never travel, and its
cache-first rule would pin an old build across an app update. And
`webContentsDebuggingEnabled` is off, because `window.BL` is still in
the shipped file and it can write to the save.

The Android project is generated and it builds — a 4.5MB debug APK with
the whole game in `assets/public/index.html`. **iOS is not done.** The
...
No analytics, no crash capture, nothing that could answer "how far did
people get" or "did it fall over on somebody's phone". A thousand
installs would have produced a thousand silences, and the money spent
getting them would have bought no information at all.

`18-telemetry.js` is the seam, built like `17-billing.js` and for the
same reason: the shape exists so that choosing a vendor is a change to
one function rather than to the game. Nothing in it talks to a network.
There is no endpoint, no key, no third-party script, and the bundle
still has no runtime dependency — `sink` is null, and events go into a
ring buffer on the device and nowhere else.

Two rules underneath, and they are not negotiable the day a sink is
wired up. Nothing identifying goes in: no pet names, no free text, no
device or advertising id. `install` is a random number the file makes up,
it lives only in the save, and wiping the save forgets it. The value
coercion is where that is enforced rather than promised — a number stays
a number, a short token stays a token, and anything else is dropped, so
a pet called after its owner cannot reach a payload by accident. And the
switch in Settings governs whether anything is *written*, not only
whether it is sent, which is what makes "no data collected" an honest
answer while there is no sink and a truthful one afterwards.

What it counts is the funnel that was missing: where onboarding is
abandoned, which level is started and how it ended, and — the number
worth the most — how far short a loss fell. A level people lose at five
percent of the goal is tuned. One they lose at sixty is a wall, and the
difference is invisible from the outside.

## The gate that cried wolf

`npm test` failed about one run in four, and it had nothing to do with
the build. The cared-for difficulty curve was gated at a 93% ceiling and
sits at 90%, measured across three runs of the current levels and three
of the originals: 90/90/90 against 90/89/89. The 88 and the 94 that
bracket those are the same distribution, not a drift.

Five games a level over sixty levels is three hundred games, and the
spread that leaves is about four points either side. A hard ceiling four
points above the mean is a coin flip wearing a gate's clothes, and it is
worse than no gate at all: a suite that cries wolf gets ignored, and the
next real drift walks through behind it.

Widening the band alone would have hidden the noise rather than reduced
it, so the gate does both — nine games a level, which pulls the spread in
by about a quarter, and a ceiling at 96, still far below "the pet plays
it for you", which is the thing it exists to catch.

## Somewhere to be walking to

The lane was sixty levels arranged as a corridor. The rhythm was already
there — `isGate` marks every tenth — but nothing on the screen said where
you were or what was ahead, so the middle of the game read as flat
however well the curve underneath it was tuned. Difficulty was never the
churn risk here. Boredom was.

Six named stretches of ten, and the work is done by the ground rather
than by a sign: the lane visibly changes colour under you as you cross
into a new one, drawn as one gradient down the whole scroll so it is a
walk rather than a step. A banner pinned to the top of the view says
which stretch you are looking at and how much of it is behind you, and
the level card says it too. Past sixty the run is generated and so are
the names — it keeps walking, and says which time round it is.

## The first minute was made of text

A new player met the level card before they had made a single swap: a
move budget, a collect goal, the pet's name and ability, a blocker count,
four perk chips, a charge line, a booster offer and a heart cost. About
ten ideas, none of which meant anything yet. The steepest part of the
whole game was minute one.

The opening levels go straight to the board now, and the card starts
carrying itself from level 8, once there is something to hang it on.
Nothing is hidden that the player could act on — the boosters it offers
are not owned yet, and the perks apply whether or not the card lists
them.

This is also where `test/integration.html` earned its keep twice over.
The check that nothing on the board arrives unexplained failed
immediately: mud, crates and rescues all first appear before level 8. It
turned out the *game* was fine — `maybeTutorial` explains all three and
the skip path still calls it — but the check drove the card's Start
