...
## App Store — its own fields

Apple asks for three things Play does not, each with its own limit, and
each shown in a different place: the subtitle sits under the name on the
product page, the promotional text can be changed without a review, and
the keywords are never displayed at all. The description above is used
unchanged — it is inside Apple's 4000 and says nothing Android-specific.
...
Use the Play full description above, unchanged. It is within Apple's
4000 and contains no Android-specific wording.
