...
const SFX = {
  ...
  tap() {
    click(6000, .05);
    tone({ f: 620, f2: 780, dur: .07, type: 'triangle', gain: .075, send: .1 });
  },
  select() {
    click(5600, .06);
    tone({ f: 520, f2: 700, dur: .1, type: 'triangle', gain: .105, send: .14 });
    noiseBurst({ fc: 3400, dur: .04, gain: .038, q: 2 });
    thump(150, .045, .07);
  },
  bad() {
  ...
  uiTick() { click(7200, .035); tone({ f: 900, dur: .035, type: 'square', gain: .04 }); },
