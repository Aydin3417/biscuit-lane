...
/* When the game is one file with nothing beside it — which is how it is
   most often handed to somebody — the manifest and the icon are simply
   not there. The game does not care; it 404s them and runs. But iOS will
   still add it to a home screen from the meta tags alone, and it would
   use a screenshot of the page as the icon.

   So if the real files are missing, both are rebuilt at runtime from the
   same drawLogo() that paints the top bar: the icon as a data URI, the
   manifest as a blob, which is same-origin and so allowed where a data
   URI would not be. Costs nothing when the files are there, and nothing
   in the file when they are not.

   What this cannot do is offline. A service worker has to be fetched
   from a real same-origin path; there is no way to register one from a
   blob. Offline needs the game to be served, and that is the honest
   limit of a single file. */
function standInIcon(px) {
  try {
...
function standInApp() {
  const icon = standInIcon(180);
  if (icon) {
    const link = document.querySelector('link[rel="apple-touch-icon"]');
    if (link) link.href = icon;
  }
  const big = standInIcon(512);
  ...
  try {
    ...
    man.href = URL.createObjectURL(new Blob([JSON.stringify(body)], { type: 'application/manifest+json' }));
  } catch (e) { /* the meta tags alone still work */ }
}

function installApp() {
  themeColorSync();
  /* only if the real ones are not being served */
  const man = document.querySelector('link[rel="manifest"]');
  if (man) {
    fetch(man.href, { method: 'GET' })
      .then(r => { if (!r.ok) standInApp(); })
      .catch(() => standInApp());
  }
