...
1. **Deuteranopia.** Beagle blue and Sable purple measure 6.6 dE apart
   for a deuteranope — indistinguishable by colour. Silhouettes carry
   it. Should the six tile hues be re-chosen so that colour carries it
   too, and what is lost if they are? Alternatively, should the symbol
   layer be on by default?
